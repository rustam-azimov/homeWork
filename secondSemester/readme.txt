...
2)Релизовать программу-калькулятор со следующим графическим интерфейсом (аргументы — QSpinBox, операция — QComboBox, значение — QLineEdit, при изменении значения аргумента или операции результат пересчитывается автоматически):

3)Реализовать продвинутый кнопочный калькулятор со следующим интерфейсом (для решения задачи использовать класс QSignalMapper).
